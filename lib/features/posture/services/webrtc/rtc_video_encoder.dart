...
// - Offers an SDP bit-hinting helper for x-google-* on H.264 fmtp lines
//
...
class RtcVideoEncoder {
  ...
  Future<void> applyTo(RTCRtpTransceiver transceiver) async {
    ...
    await _applySenderLimits(transceiver.sender,
        maxKbps: maxBitrateKbps, maxFps: idealFps,
        enableSimulcast: enableSimulcast, simulcastScales: simulcastScales);
  }
    ...
    try {
      ...
      bool isLowLatencyH264(RTCRtpCodecCapability c) {
        ...
        return mime == 'video/h264'
            ...
            && (
              // common "constrained-baseline" / "baseline"
      ...
      if (forceCodec != null && forceCodec.trim().isNotEmpty) {
        ...
        List<RTCRtpCodecCapability> pick() {
          switch (fam) {
            case 'H264': return [...h264LL, ...h264.where((c) => !h264LL.contains(c))];
            case 'H265':
        ...
        order.addAll(h264LL);
        order.addAll(h264.where((c) => !h264LL.contains(c)));

        if (preferHevc && h265.isNotEmpty) {
          order = [...h265, ...order]; // put HEVC before H.264 families
        } else {
          // push HEVC to the end (or omit if you want to avoid HEVC)
          order.addAll(h265);
        }
  ...
  // ────────────────────────────────────────────────────────────────────────────
  // SDP hinting (for H.264 x-google-*)
  // ────────────────────────────────────────────────────────────────────────────

  /// Inject or update x-google-start/min/max-bitrate on H.264 fmtp lines that
  /// have packetization-mode, to nudge initial rate control.
  static String mungeH264BitrateHints(String sdp, {required int kbps}) {
    // Split safely for \r\n / \n / \r without using raw string pitfalls.
