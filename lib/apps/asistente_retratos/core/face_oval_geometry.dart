...
const double kOvalCircularity = 0.3;

Rect faceOvalRectFor(
  Size size, {
  double circularity = kOvalCircularity,
  double scale = kOvalScale,
}) {
  final s = scale.clamp(0.0, double.infinity).toDouble();
  final ovalW0 = size.width * kOvalWFrac * s;
  final ovalH0 = size.height * kOvalHFrac * s;
...
List<Offset> faceOvalPointsFor(
  Size size, {
  int samples = 120,
  double circularity = kOvalCircularity,
  double scale = kOvalScale,
}) {
  final r = faceOvalRectFor(size, circularity: circularity, scale: scale);
...
Path faceOvalPathFor(
  Size size, {
  double circularity = kOvalCircularity,
  double scale = kOvalScale,
}) =>
    Path()
      ..addOval(faceOvalRectFor(size, circularity: circularity, scale: scale));
